...
class AppCloud {
    ...
    constructor(username, password, region, appType, appDeviceId, appSessionId, appRegion, appEndpoint, loginCallback) {
        ...
        if (appType === 'tuya_smart') {
            this.apiKeys = {
                ttid: 'tuya',
                key: process.env.TUYA_SMART_KEY,
                certSign: process.env.TUYA_SMART_CERTSIGN,
                secret2: process.env.TUYA_SMART_SECRET2,
                secret: process.env.TUYA_SMART_SECRET,
            };
        } else if (appType === 'ledvance') {
            this.apiKeys = {
                ttid: 'sdk_tuya@fx3fvkvusmw45d7jn8xh',
                key: process.env.LEDVANCE_KEY,
                secret: process.env.LEDVANCE_SECRET,
                secret2: process.env.LEDVANCE_SECRET2,
                certSign: 'A',
            }
        } else if (appType === 'sylvania') {
            this.apiKeys = {
                ttid: 'sdk_tuya@creq75hn4vdg5qvrgryp',
                key: process.env.SYLVANIA_KEY,
                secret: process.env.SYLVANIA_SECRET,
                secret2: process.env.SYLVANIA_SECRET2,
                certSign: 'A',
            }
        } else {
            this.apiKeys = {
                ttid: 'smart_life',
                key: process.env.SMART_LIFE_KEY,
                secret: process.env.SMART_LIFE_SECRET,
                secret2: process.env.SMART_LIFE_SECRET2,
                certSign: process.env.SMART_LIFE_CERTSIGN,
            }
        }
