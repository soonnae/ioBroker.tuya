...
class TuyaOpenMQ {
    ...
    async _loop_start() {
        ...
        while (this.running) {
            ...
            try {
                ...
                // ignore error, handled next lines
            }
            if (!res || !res.success) {
                this.stop();
                break;
    ...
    _onMessage(client, mqConfig, topic, payload) {
        ...
        if (message.data && message.data.t === undefined) {
            message.data.t = message.t;
        }
    ...
    _decodeMQMessage(data, password, t) {
        ...
        var cipher = Crypto.createDecipheriv('aes-128-gcm', key, iv_buffer, { authTagLength: GCM_TAG_LENGTH });
        //setAuthTag buffer
        ...
        msg += cipher.final('utf8'); // Ensure final is called to check the auth tag
